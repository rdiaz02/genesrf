...
dm90 <- expand.grid(dataset = datanames, num.node = nodes, reps = c(1))
walltime <- rep(NA, nrow(dm90))

for(i in 1:nrow(dm90)) {
    cat("\n Doing i ", i, ".  dataset = ", dm90[i, 1],
        " . num.node = ", dm90[i, 2], "\n")
    try(stopCluster(TheCluster))
    try(rm("TheCluster"))
    try(rm(TheCluster))
    walltime[i] <- timef(dm90[i, 1], dm90[i, 2])
    cat("\n       This walltime was ", walltime[i], "\n")
    save.image()
}

dm90$walltime <- walltime

save(file = "dm90.RData", dm90)
